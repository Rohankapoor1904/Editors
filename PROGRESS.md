...
| **R23.3** | Bridge Rust | Task routes /prompt /tool /action /connect /pending /result /heartbeat | `partial` | `blocked` | — | Code written: 7 routes mirroring dev-plugin semantics (Bearer on POSTs, 400s, 20/20/15s timeouts, oneshot handshake, waiter cleanup); +3 unit tests (`split_outcome`, id shape). Verified: `rustfmt` clean; full `npm test` gate passes (no new invokes — server side only). NOT verified: `cargo check`/`cargo test` — toolchain probe 2026-09-22: rust-lld + clang present, `.cargo/config` only loads with CWD=src-tauri, but `vswhom-sys` build script (via tauri-build→winres) needs `windows.h`, absent from xwin snapshot and unusable from MinGW trees (builtin-header conflicts). Needs full MSVC/SDK host. Deps: R23.2 |
...
- **R23 exit:** In progress. R23.1 (TS transport), R23.2 (scaffold, host-verified) and R23.4 (BridgePanel UI) `done` and merged in PR #92. R23.3 (task routes) in progress; R23.5 (desktop e2e) todo.

## Next agent

PR #92 merged cleanly into `main` (`1f8e5e0`). R21.1–R22.6 + R23.1 + R23.4 are marked `done` (all `real` with passing behavioural tests). R23.2 stays `blocked` until an MSVC host runs `cargo check`. Next code task: R23.3 (task routes: `/prompt`, `/tool`, `/action`, `/connect`, `/pending`, `/result`, `/heartbeat` in `src-tauri/src/bridge_server.rs`) on an MSVC-tooled host.
