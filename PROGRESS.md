...
| **R22.2** | Packaging | Bundle ONNX/Whisper models + missing-model UX | `missing` | `in_progress` | opencode | 2026-09-22 claim. Scope note: `cargo check` baseline FAILS here (no MSVC `link.exe`) → no Rust edits; resources config + frontend guidance only, Rust resource-dir wiring left as explicit follow-up. Deps: none |
