...
| **R0.3** | Explicit `demo`/`live` runtime mode; remove silent mock fallbacks | R0 | `done` | `npm test` → 35 passed | `src/services/*`, `src/engine/*`, `src/components/TopBar.tsx` | R0.1 |
...
| R0.3 — runtime mode & throw-on-stub in live mode verified | `npm run test` → `✓ src/__tests__/runtimeMode.test.ts (12 tests)`, `Tests 35 passed (35)` | 2025-09-15 |
