...
| **R21.1** | Agent bridge | Bridge connection config + honest production state | `partial` | `in_progress` | opencode | Impl: URL+token configurable (`agentBridge.ts`), `CINECRAFT_AGENT_TOKEN` auth + `bridge:dev-middleware` in plugin status, explicit unavailable-in-production UI (`AIPromptConsole.tsx`), store fields. Test: `src/services/__tests__/agentBridgeConfig.test.ts` 6/6 pass; `tsc` clean; `eslint` clean; `npm run build` 10.43s. NOT `done`: `npm test` gate fails on pre-existing tracked `Launch_CineCraft.bat` root-clutter violation (outside R21.1 scope); PR not opened. Missing subset: native production transport (deferred per ADR-008). Deps: none |
...
| **R22.1** | Render | Remove placeholder caption WGSL from GPU pipeline | `partial` | `in_progress` | opencode | Impl: deleted `caption.wgsl` + accessor + group(3) plumbing; pipeline compiles grade-only; canvas captions unchanged; `({} as any)` → `()`; remaining `as any` verified load-bearing (@webgpu/types friction, documented). Test: renderer 2/2, caption 6/6 (+absence pin); gate fire-drill proved R22.1 failure + clean-tree zero new errors; `tsc`/`eslint` clean; build 5.49s. NOT `done`: full gate red on pre-existing `.bat`; PR not opened. Missing: real GPU text layout (canvas is the renderer). Deps: none |
| **R22.2** | Packaging | Bundle ONNX/Whisper models + missing-model UX | `partial` | `in_progress` | opencode | Impl: `bundle.resources` ships both models; `modelErrors.ts` maps missing-model errors to actionable guidance (no invented URLs); both editors surface it. Test: `modelErrors` 6/6; `tsc`/`eslint` clean; build 5.55s; tauri.conf JSON-valid. NOT `done`: `cargo check` impossible here (no MSVC `link.exe`, baseline fails too) → Rust resource-dir wiring untouched; full gate red on pre-existing `.bat`; PR not opened. Discovered (not caused): `TranscriptEditor.test.tsx` R13.3 fails identically on pristine `.kilo` copy — pre-existing, out of scope. Missing: Rust-side resource resolution. Deps: none |
...
| **R22.6** | Editorial | Serialize durations honestly | `real` | `in_progress` | opencode | Impl: `parseAssetDuration()` parses `value/rate` (validated) + `HH:MM:SS[.mmm]` at project fps; unparsable omitted (schema-optional), never invented; deserialize tolerates missing as explicit `''`. Test: `serializeDurations` 6/6, `schema` 3/3 round-trip incl. golden; `tsc`/`eslint` clean; build 6.06s. NOT `done`: full gate red on pre-existing `.bat`; PR not opened. Deps: none |
| **R23.1** | Bridge TS | Sidecar transport: status fetch + discovery + store | `missing` | `in_progress` | opencode | 2026-09-22 claim. Deps: none |
| **R23.2** | Bridge Rust | Axum scaffold + /status + /timeline + get_bridge_info (unverified here) | `missing` | `todo` | — | Deps: R23.1 |
| **R23.3** | Bridge Rust | Task routes /prompt /tool /action /connect /pending /result /heartbeat | `missing` | `todo` | — | Deps: R23.2 |
| **R23.4** | Bridge UI | Sidecar port/token/status panel | `missing` | `todo` | — | Deps: R23.1 |
| **R23.5** | Bridge e2e | Desktop end-to-end verification on Tauri host | `missing` | `todo` | — | Deps: R23.3, R23.4 |
